...
\usepackage{outline}
\usepackage{pgfplots}
\usepackage[binary-units = true]{siunitx} \sisetup{load-configurations=abbreviations} %use correctly typed SI units
\usepackage{subfig}
\usepackage{tablefootnote}
\usepackage{tabularx}%tables with variable space
\usepackage[noindentafter]{titlesec}
\usepackage{thumbpdf} %thumbnails
\usepackage{xcolor} %must be loaded before tikz
...
\usepackage[noabbrev]{cleveref} % the following loading order must be obeyed: varioref, hyperref, cleveref (assuming all three are present) 
